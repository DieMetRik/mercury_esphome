...
class Mercury : public PollingComponent, public UARTDevice {
	...
	void setup() override
	{
		this->set_update_interval(1000);	// Периуд обновления сенсора
	...
	void update() override
	{
		...
		if (step == 2){
			main_uart_read(electrical_parameters_F);
		}

		if (step == 3){
			main_uart_read(dtime);
		}
		...
		if (Re_buf[4] == 0x21){
			if (crc_check(Re_buf, 11)==1) {			// Проверяем CRC пришедшего сообщения
				...
				//------------------
				dt_str = yy_s + "-" + mon_s + "-" + dd_s + " " + hh_s + ":" + mm_s + ":" + ss_s;	// СФОРМИРОВАТЬ СТРОКУ ВРЕМЕНИ
			}
			else
			{
			crc_good[3] = 0;
